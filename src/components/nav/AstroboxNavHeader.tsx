...
import styles from "./AstroboxNavHeader.module.css";
...
const isInternalHref = (item: NavHeaderItem): boolean => {
  if (!item.href || item.external) return false;
  return !/^(?:[a-z][a-z\d+.-]*:|\/\/)/i.test(item.href);
};

const renderDesktopItem = (item: NavHeaderItem, className: string, isActive: boolean) => {
  const resolvedClassName = isActive ? `${className} ${styles.navLinkActive}` : className;

  if (item.href) {
    if (isInternalHref(item)) {
      return (
        <Link href={item.href} className={resolvedClassName} aria-label={item.ariaLabel} onClick={item.onClick}>
  ...
  return (
    <button type="button" className={resolvedClassName} aria-label={item.ariaLabel} onClick={item.onClick}>
      <span className={styles.desktopLabel}>{renderItemLabel(item)}</span>
    </button>
  );
};

const renderMobileItem = (item: NavHeaderItem, className: string, isActive: boolean) => {
  const resolvedClassName = isActive ? `${className} ${styles.menuLinkActive}` : className;
...
const NavHeader: React.FC<NavHeaderProps> = ({
  ...
  return (
    <div className={cx(styles.header, className)} data-nav-header data-variant={variant}>
      {showHeaderBlur && <BlurEffect className={styles.blurEffect} position="top" intensity={100} aria-hidden />}
      <header className={styles.inner}>
        {hasNavItems && (
          <div className={styles.mobileMenu}>
            <Drawer.Root open={menuOpen} onOpenChange={setMenuOpen} shouldScaleBackground direction="top">
              <Drawer.Trigger asChild>
                <button type="button" className={styles.iconButton} aria-label={resolvedLabels.menu}>
                ...
                <Drawer.Content className={styles.menuDrawerContent}>
                  <div className={styles.menuDrawerInner}>
                    <div className={styles.menuDrawerHeader}>
                      <Drawer.Title className={styles.menuTitle}>{resolvedLabels.menu}</Drawer.Title>
                      <Drawer.Close asChild>
                        <button
                          type="button"
                          className={cx(styles.iconButton, styles.menuCloseButton)}
                    ...
                    <nav className={styles.menuNav} aria-label={resolvedLabels.menu}>
                      {navItems.map((item) => {
                        const key = item.id ?? item.href ?? item.ariaLabel;
                        const isActive =
                          item.active ??
                          Boolean(item.href && !item.external && isMenuItemActive(currentPath, item.matchPath ?? item.href));

                        return (
                          <Drawer.Close asChild key={key}>
                            {renderMobileItem(item, styles.menuLink, isActive)}
                          </Drawer.Close>
                        );
                      })}
                    </nav>
                  ...
                  <div aria-hidden className={styles.menuDrawerHandle} />
                </Drawer.Content>
              </Drawer.Portal>
            </Drawer.Root>
          </div>
        ...
        <div className={styles.centerColumn}>
          <div className={styles.centerTrackWrapper}>
            <div className={styles.slide}>
              <Link className={styles.logotypeWrapper} href={homeHref} aria-label={resolvedLogoAriaLabel}>
                {brandContent}
              </Link>
            </div>
          </div>
          ...
          {hasNavItems && (
            <>
              ...
              <nav className={styles.desktopNav} aria-label={resolvedLabels.menu}>
                  ...
                  {navItems.map((item) => {
                    ...
                    return (
                      <div className={styles.navItem} key={key}>
                        {renderDesktopItem(item, styles.navLink, isActive)}
                      </div>
                    );
                  })}
                </div>
